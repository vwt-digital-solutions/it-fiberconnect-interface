...
def send_bytestream_to_filestore(bytesIO, filename, bucket_name):
    ...
    if filename.endswith(".xlsx"):
        blob.upload_from_string(bytesIO.getvalue(), content_type=config.MEDIA_TYPE)
    else:
        blob.upload_from_string(bytesIO, content_type=config.MEDIA_TYPE)
...
def preprocessing(bucket_name, blob_name):
    ...
    for key, value in config.COLUMN_COMBINE.items():
        df[key] = df[value].astype(str).apply("_".join, axis=1)

    # !! The double ID are removed. Only differnece between the duplicates is the
    # patch column. Therefore, first is kept and the rest is removed. !!
    mask = df.duplicated(config.UID, keep="first")
    df = df[~mask]
    ...
    df = df[config.COLUMN_MAPPING.values()]
    ...
    return dict(status="success", message="file succesfully processed", file=bytesIO)
...
def file_processing(data, context):
    ...
    if hasattr(config, "FILEPATH_PREFIX_FILTER"):
        if not filename.startswith(config.FILEPATH_PREFIX_FILTER):
            ...
            return dict(stutus="succes", message="file is skipped")
